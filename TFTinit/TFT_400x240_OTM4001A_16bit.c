...
void TFTLCD_ShowString(uint32_t x,uint32_t y,char *p,uint32_t fColor, uint32_t bColor,uint32_t timeDelay)
{
...
void TFTLCD_ShowData(uint32_t x,uint32_t y,int32_t i32Data,uint32_t fColor, uint32_t bColor)
{
	...
	{
		TFTLCD_ShowString(x,y,"Error:Out of range!",RED,WHITE,0);
		return;
	}
	if(i32Data < 0)
	{
	...
	Data[4] += '0';
	Data[5] = '\0';
	TFTLCD_ShowString(x,y,Data,fColor,bColor,0);
	;
}
...
void TFTLCD_TwinkleString(uint32_t x,uint32_t y,char *p,uint32_t fColor, uint32_t bColor,uint32_t timeDelay){
	TFTLCD_ShowString(x,y,p,fColor,bColor,1);
	SysCtlDelay(timeDelay);
	TFTLCD_ShowString(x,y,p,bColor,bColor,1);
	SysCtlDelay(timeDelay);
}

void TFTLCD_ColorChangeChar(uint32_t x,uint32_t y,char *p,uint32_t fColor1, uint32_t fColor2,uint32_t bColor,uint32_t timeDelay){
	TFTLCD_ShowString(x,y,p,fColor1,bColor,1);
	SysCtlDelay(timeDelay);
	TFTLCD_ShowString(x,y,p,fColor2,bColor,1);
	SysCtlDelay(timeDelay);
}
...
void TFTLCD_showLed(uint32_t x,uint32_t y,uint32_t fColor,uint32_t bColor,uint32_t mode){
	...
	case 9:{
		...
		TFTLCD_ShowString(x-r/2-a-4,y+r/2+a+8,"o",fColor,bColor,1);SysCtlDelay(80000);
		TFTLCD_ShowString(x-r-4,y+8,"o",fColor,bColor,1);SysCtlDelay(80000);
		TFTLCD_ShowString(x-r/2-a-4,y-r/2-a+8,"o",fColor,bColor,1);SysCtlDelay(80000);Button_run();}break;
	default:break;
	}
}
